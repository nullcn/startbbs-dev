...
class Forum extends SB_controller
{
	...
	public function view ($fid=1)
	{
		...
		//关键字tag
		if($content['keywords']){
			$data['tag_list'] = $data['keywords'] = explode(',',$content['keywords']);
		}
		//相关贴子
		if(isset($data['tag_list'])){
			$this->load->model('tag_m');
			$data['related_forum_list'] = $this->tag_m->get_related_forums_by_tag($data['tag_list'],10);
	...
	public function add()
	{
		...
		if(!$this->auth->is_login()) {
			redirect('user/login/');
		} elseif(time()-$user['lastpost']<10){
			$this->session->set_flashdata('error', '发帖最小间隔时间是10秒!');
			exit;
		} else {
	...
	private function validate_add_form(){
		...
		$this->form_validation->set_rules('content', '内容' , 'trim|required|min_length[6]|max_length[5000]');
